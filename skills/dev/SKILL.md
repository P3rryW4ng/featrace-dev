...
For scan and scoped feature work read `core/references/module-context.md`. Establish a project module catalog at initial scan; when available, select feature modules and inspect scoped dossiers/dependencies/callers before code changes. Refresh stale module evidence and adopt module_context_required for new scoped work. `/dev scan --module <label>` uses this scoped workflow; unknown topology requires broader investigation. Dossiers are source-reviewed implementation knowledge, not acceptance or product authority.
