...
Before business-code edits and again before check, read `core/references/historical-regression.md`. Sync related verified-fix candidates from registered modules and code paths, then disposition each candidate as retest or not applicable with evidence. Never reopen historical fixes or claim semantic completeness automatically.
