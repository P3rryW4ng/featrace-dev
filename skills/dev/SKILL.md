...
| classify [ID] | Confirm the business capability and each involved code module's role; legacy labels remain readable |
...
For scan and scoped feature work read `core/references/module-context.md`. Establish a project module catalog at initial scan; Android scan first writes static Gradle findings to `modules/candidates.json`/`.md`. Treat every discovered module and dependency as pending: inspect its file/line evidence and parsing gaps before copying an accepted relationship into the reviewed catalog. Candidate output never confirms ownership, satisfies a dossier gate, or proves a runtime call. When a catalog is available, select feature modules and inspect scoped dossiers/dependencies/callers before code changes. New workflow v3 workspaces start with pending `module_scope`: after PRD analysis propose a business capability plus evidence-backed module roles, ask the user to confirm ambiguous ownership, and record the result with `feature_scope.py`. Do not force one module when a capability crosses an owner, host, provider or consumer. Develop/check block until scope is confirmed or specifically not applicable. Refresh stale module evidence before develop and after affected code changes. `/dev scan --module <label>` uses this scoped workflow; a completed scan with a module catalog also regenerates `modules/graph.json` and `graph.md` through `module_context.py graph`. Unknown topology requires broader investigation. Dossiers and graphs are source-reviewed navigation aids, not acceptance, product authority or a complete runtime call graph.
...
5. Create a feature using `bash core/scripts/init-feature.sh <ID> <PRIMARY-DOCUMENT-OR-HTML> <PROJECT> [--source <ADDITIONAL-FILE> ...]`. `<ID>` is the product/work-item identifier supplied by the user; never invent or silently normalize it. If it is missing, ask once before creating files. Workflow-local requirement/task/decision/fix IDs remain separate. Only one source part is required; HTML can be primary. Script preserves source bytes and extension. Interpret Word/PDF through available readers before decomposing; do not pretend binary input is Markdown.
6. Register and review every supplied part, including observable HTML states and actions. Missing dynamic dependencies or inaccessible interactions remain reading gaps; conflicts between document and interaction are decisions, not silent precedence choices. Populate `spec/prd-intake.json` and requirement `source_item_ids` under the PRD analysis contract. Review the original against the spec, then record the full review with `core/scripts/review-prd.py`. After the requirement meaning is known, consult the module catalog and relevant code, propose capability/module roles with evidence, and ask the user only about unresolved ownership or scope. Record confirmed scope before develop; do not rerun a full scan when a current catalog plus scoped investigation is sufficient. If reading gaps or pending items remain, record only a partial draft review using `--stage draft` and state the gaps in the notes; this never satisfies develop/check. Empty/missing/stale intake or pending module scope blocks develop/check, including legacy workspaces that explicitly adopt the new fields; draft only warns about incomplete work.
