...
regression-review.json   Canonical candidate/disposition/result record for related verified historical fixes
regression-review.md     Generated review view; never edit as authority
task-review.json         Canonical task semantic snapshot/review/history; excludes progress fields
task-review.md           Generated task review view; never edit as authority
```
...
Business-code work adopts `feature.history_regression_required: true` when `regression_review.py sync` creates the feature-local historical regression review. Schema v2 keeps active candidates/dispositions separate from read-only `history`; superseded entries preserve prior evidence but cannot satisfy current gates. Candidates come only from registered modules and paths and therefore are reminders, not a complete semantic dependency graph. See [historical-regression.md](historical-regression.md).

Task generation or later semantic task edits adopt `feature.task_review_required: true` through `task_review.py sync`. The digest includes every task field except status, execution/test evidence, verification, implementation revision, timestamps and assignee. Changed task IDs and their old/new requirement links define the required focused review scope. See [task-semantic-review.md](task-semantic-review.md).
