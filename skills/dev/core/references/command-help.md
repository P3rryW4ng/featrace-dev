...
- develop: optional positional ID after selection; starts/continues implementation. Includes impact investigation, related verified-fix candidate review and validation, not just record generation. Unresolved product choices need clarification.
...
- check: optional positional ID after selection. Refreshes historical-fix candidates, requires results for selected retests, generates acceptance rows, runs selected mapped checks and lists remaining manual work; may execute builds/tests. Builds do not automatically pass UI acceptance. A structural pass alone is not delivery completion. status only reports existing state.
