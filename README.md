...
当前源码版本 **0.5.7**：通用工作流 + Android 适配 + Generic 适配。iOS/Web/后端可用通用流程，但没有专属自动化适配器。此工具由 Agent 执行需求理解和编码，脚本负责部分确定性检查；不能保证零遗漏或零 bug。
...
日常只需优先记住三个入口：`prd` 创建新需求，`next` 继续当前需求，`status` 只读查看进度。`next` 会根据描述和当前记录自动进入补设计、补接口、澄清、修订、修复、开发或验收流程；现有精确命令仍可直接使用。
