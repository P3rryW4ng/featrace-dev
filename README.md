...
当前源码版本 **0.5.16**：通用工作流 + Android 适配 + Generic 适配。Android scan 可生成待人工确认的 Gradle 模块/依赖候选；iOS/Web/后端可用通用流程，但没有专属自动化适配器。此工具由 Agent 执行需求理解和编码，脚本负责部分确定性检查；候选关系不是完整调用图，也不能保证零遗漏或零 bug。
...
| 业务能力与模块职责确认 | `/dev classify OGFR-2577` | `$dev classify OGFR-2577` |
...
任务含义另有局部复核：标题、描述、范围或关联需求改变时，只核对变化任务及其新旧关联需求；状态、负责人和测试证据等进度变化不触发。摘要只能发现内容变化，实际是否符合需求仍由 Agent 对照证据判断。
...
扫描文件策略：固定当前基线、草稿校验后发布、内容变化才备份、默认保留最近 3 份托管历史。verify 会区分 HEAD、清单、登记证据和工作树路径；仅刷新已审阅的新 HEAD 不备份相同正文。详见使用与迭代指南。
...
PRD 可由文档、HTML 交互稿或两者组成；使用 `/dev prd <首个文件> --feature <产品需求编号> --source <附加文件>`。产品编号必须由用户提供，Skill 不生成。PRD 分析后，Agent 根据项目模块目录和代码证据推荐业务能力及多模块职责，由用户确认有歧义的范围后才能进入开发。详细边界见 [使用指南](docs/usage.md)。
