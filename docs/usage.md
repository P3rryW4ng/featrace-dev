...
适用版本：0.5.7；最后核对：2026-09-21。下图描述当前 Agent 工作流程；结构校验由脚本辅助，资料理解、冲突判断和最终完成条件仍需 Agent 结合证据核对。
...
- **日常统一入口**：已有需求选中后，可直接用 `/dev next <你现在要做的事>`。Agent 先说明路由，再执行原有流程；不只是返回一条命令建议。新需求仍用 `prd --feature`，新会话仍需 `use`。图片是设计素材还是故障截图，按它的用途而非文件类型分流。
...
常规开发不需要背诵上述全部精确命令。选中需求后可直接说：`/dev next 我补充了一批图片，路径是……`、`/dev next 返回后表单被清空`或 `/dev next 继续开发`。无描述的 `/dev next` 会先读状态，只在下一步唯一且已获授权时执行；否则只追问会影响产品权威的必要信息。
