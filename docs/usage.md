...
适用版本：0.5.11；最后核对：2026-09-21。下图描述当前 Agent 工作流程；结构校验由脚本辅助，资料理解、冲突判断和最终完成条件仍需 Agent 结合证据核对。
...
- **历史修复回归提醒**：无需新增命令。develop/fix/revise 在业务代码修改前同步当前模块与登记代码路径相关的历史 `verified` 修复；Agent 对每项选择重测，或写明本次为何不适用。check 前再次同步，候选、范围或历史修复内容变化会使旧结果失效；原历史 FIX 状态保持不变。候选依赖已有模块和路径登记，遗漏的依赖仍需人工扩大调查。详见[历史回归规范](../skills/dev/core/references/historical-regression.md)。
