...
适用版本：0.5.18；最后核对：2026-09-24。下图描述当前 Agent 工作流程；结构校验由脚本辅助，资料理解、冲突判断和最终完成条件仍需 Agent 结合证据核对。0.5.18 只修正追溯视图中的任务编号显示，流程与命令保持 0.5.17 的行为。
...
- **一个入口，六个内部阶段**：0.5.17 将现有流程组织为 Requirement、Scope、Build、Verify、Repair、Deliver。路由器先读当前需求状态，再只加载本阶段及直接前置规则；阶段结果写回已有需求、任务、影响、修复、验证和交付记录，不新增通用交接文件。当前仍由同一 Agent 执行，尚未启用独立 Agent/上下文；后续先用真实需求测量，再决定是否优先隔离 Verify。
