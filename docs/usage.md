...
适用版本：0.5.8；最后核对：2026-09-21。下图描述当前 Agent 工作流程；结构校验由脚本辅助，资料理解、冲突判断和最终完成条件仍需 Agent 结合证据核对。
...
- **日常统一入口**：已有需求选中后，可直接用 `/dev next <你现在要做的事>`。Agent 先说明路由，再执行原有流程；不只是返回一条命令建议。新需求仍用 `prd --feature`，新会话仍需 `use`。图片是设计素材还是故障截图，按它的用途而非文件类型分流。Claude Code 可先输入一次 `/dev` 进入当前会话、当前项目的开发模式；随后普通开发描述默认走 `next`，也可直接输入裸命令词。`dev off` 退出，切换项目或新会话后需重新进入。该状态不写文件，不保证跨上下文恢复。Claude Code 的 `/status` 是内置命令，需求状态请输入普通文字 `status` 或 `/dev status`。
