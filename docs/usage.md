...
适用源码版本：0.5.21（待发布）；最后核对：2026-09-24。下图描述当前 Agent 工作流程；结构校验由脚本辅助，资料理解、冲突判断和最终完成条件仍需 Agent 结合证据核对。0.5.19 在 Verify 开始前增加只读输入核对，不改变已有命令、记录权威或验收门禁。
...
flowchart TD
    ...
    B --> C{"基线缺失或过期？"}
    C -->|是| D["创建扫描草稿，盘点项目与核对规则<br/>Android 生成 Gradle 候选与缺口；未知目标和条件依赖保持待审<br/>条件依赖不得压成无条件目录边<br/>校验草稿后发布，变更才备份，保留 3 份"]
    ...
    G1 --> MSCOPE["查询模块目录与相关代码<br/>推荐业务能力及多模块职责<br/>用户确认歧义；不得猜测产品归属"]
    MSCOPE --> TR["同步任务语义摘要<br/>局部核对任务与关联需求/依据"]
    ...
    K --> H
    I -->|否| MODSEL{"能力与模块职责已确认？"}
    MODSEL -->|是| MODREV["复核相关模块、调用方与依赖档案<br/>过期则按证据更新"]
    MODSEL -->|否| MSCOPE
    MODREV --> IMP
    ...
    HRC --> VIN["Verify 输入核对：只读列出来源、补充资料、Git 改动<br/>和已有质量报告对应版本；不作通过判定"]
    VIN --> N["check：执行所选历史回归和本次验收<br/>记录当前版本、方法与证据"]
...
- **一个入口，六个内部阶段**：0.5.17 将现有流程组织为 Requirement、Scope、Build、Verify、Repair、Deliver。路由器先读当前需求状态，再只加载本阶段及直接前置规则；阶段结果写回已有需求、任务、影响、修复、验证和交付记录，不新增通用交接文件。0.5.19 的 Verify 输入核对只暴露可读取的事实与版本差异，退出码 0 不表示验收通过。当前仍由同一 Agent 执行，尚未启用独立 Agent/上下文；后续先用真实需求测量，再决定是否优先隔离 Verify。
...
- **模块档案与能力图**：Android 初次 scan 会从常见静态 Gradle 声明生成 `modules/candidates.json`/`.md`，列出文件/行号依据和未解析缺口。它只帮助人工建立目录，不会自动确认模块、职责或运行时调用；动态 Gradle 配置仍需阅读。0.5.20 会对外部 settings 加载和 `add(..., project(...))` 依赖给出带行号的人工复核提示；0.5.21 将未被静态 include 的依赖目标保留为待审节点与虚线边，并把可识别的变体专属依赖另列为条件候选。若正式目录把仅条件生效的关系写成无条件依赖，关系图和扫描发布会拒绝，需人工修正。确认后的代码模块目录生成 `modules/graph.json` 与 Mermaid `graph.md`，候选依赖为虚线，人工声明依赖和已确认职责为实线；`/dev scan --module wallet` 聚焦登记的目标模块、直接调用方和依赖。新需求使用用户提供的产品编号，PRD 分析后由 Agent 推荐业务能力及各模块的 owner/host/provider/consumer/shared 职责，再让用户确认歧义。红包可以是业务能力，同时由 wallet 拥有规则、chat 承载展示，不强制归为单一模块。图不是完整运行时调用图。详见 [模块规范](../skills/dev/core/references/module-context.md)。
...
归档仅适用于已完成并有真实交付依据的需求。Agent 核对验收报告及代码版本后登记；原目录、原文、修复、决策和证据保留。默认 list 显示未归档需求，历史按需读取；不会为了归档扫描全项目。新工作流的 classify 保存业务能力和多模块职责，变更旧标签会使已确认职责回到 pending，防止错误沿用；旧记录的多选标签继续兼容。
