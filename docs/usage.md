...
适用版本：0.5.15；最后核对：2026-09-22。下图描述当前 Agent 工作流程；结构校验由脚本辅助，资料理解、冲突判断和最终完成条件仍需 Agent 结合证据核对。
...
flowchart TD
    ...
    B --> C{"基线缺失或过期？"}
    C -->|是| D["创建扫描草稿，盘点项目与核对规则<br/>建立模块目录并生成模块/能力关系图<br/>校验草稿后发布，变更才备份，保留 3 份"]
...
- **模块档案与能力图**：初次 scan 建立代码模块目录，并生成 `modules/graph.json` 与 Mermaid `graph.md`；`/dev scan --module wallet` 聚焦登记的目标模块、直接调用方和依赖。新需求使用用户提供的产品编号，PRD 分析后由 Agent 推荐业务能力及各模块的 owner/host/provider/consumer/shared 职责，再让用户确认歧义。红包可以是业务能力，同时由 wallet 拥有规则、chat 承载展示，不强制归为单一模块。图只包含声明依赖和已确认职责，不是完整运行时调用图。详见 [模块规范](../skills/dev/core/references/module-context.md)。
